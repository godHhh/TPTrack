...
class LinkData(Dataset):
    ...
    def initialize(self):
        ...
        for seqid, seq in enumerate(SEQ['train'], start=1):
            ...
            # path_gt = join(self.root, '{}/gt/gt.txt'.format(seq)) date:220217
    ...
    def __getitem__(self, item):  # item为随机索引
        info = self.id2info[self.ids[item]]  # ID为item的轨迹
        numFrames = info.shape[0]
        if self.mode == 'train':
            idxCut = randint(self.minLen // 3, numFrames - self.minLen // 3)  # 随机裁剪点
            # 样本对儿
            info1 = info[:idxCut + int(normalvariate(-5, 3))]  # 为索引添加随机偏差 -5为均值 3为标准差
