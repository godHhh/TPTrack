    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
       ...
       """
        ...
        if self.reduction == 'mean':
            newl = torch.mean(new_loss)  # the batch and channel average
        elif self.reduction == 'sum':
            newl = torch.sum(new_loss)  # sum over the batch and channel dims
...
class NewBCELoss(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
       ...
       """
        ...
        new_loss = self.bce_loss + self.epsilon * (1 - pt)
